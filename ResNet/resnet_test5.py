...
def resnet_conv_stack(input, num_stack_layers, num_filters):
    assert(num_filters >= 0)
    l = input
    for _ in range(num_stack_layers):
        l = resnet_conv(l, num_filters)
    return l
...
def create_cifar10_model(input, num_stack_layers, num_classes):
    ...
    r1 = resnet_conv_stack(conv, num_stack_layers, c_map[0])

    r2_1 = resnet_basic_inc(r1, c_map[1])
    r2_2 = resnet_conv_stack(r2_1, num_stack_layers-1, c_map[1])

    r3_1 = resnet_basic_inc(r2_2, c_map[2])
    r3_2 = resnet_conv_stack(r3_1, num_stack_layers-1, c_map[2])
