...
def conv_dw(input, fillter_size, num_filters, strides=(1,1), init = he_normal()):
    r = Convolution(fillter_size, num_filters, activation = None, init = init, pad = True, strides = strides, bias = False, groups = 1)(input)
...
def mobilenet_basic(input, num_filters):
    c1 = conv_dw(input, (3,3), input.shape[0])
...
def mobilenet_basic_inc(input, num_filters, strides = (2,2)):
    ...
    c1 = conv_dw(input, (3,3), n, strides)
