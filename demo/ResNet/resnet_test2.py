...
def resnet_basic_constant_inc(input, num_filters, strides=(2, 2)):
    c1 = conv_bn_relu(input, (3, 3), num_filters, strides)
    c2 = conv_bn(c1, (3, 3), num_filters, bn_init_scale=1)
    s = conv_bn(input, (1, 1), num_filters, strides) # Shortcut
    p = c2 + s
...
def resnet_basic_constant_stack(input, num_stack_layers, num_filters):
    assert(num_stack_layers >= 0)
    l = input
    for _ in range(num_stack_layers):
        l = resnet_basic_constant(l, num_filters)
    return l
...
def create_cifar10_model(input, num_stack_layers, num_classes):
    ...
    r1 = resnet_basic_constant_stack(conv, num_stack_layers, c_map[0])

    r2_1 = resnet_basic_constant_inc(r1, c_map[1])
    r2_2 = resnet_basic_constant_stack(r2_1, num_stack_layers-1, c_map[1])

    r3_1 = resnet_basic_constant_inc(r2_2, c_map[2])
    r3_2 = resnet_basic_constant_stack(r3_1, num_stack_layers-1, c_map[2])
